...
class Employee_CommentsAccess extends MY_Controller {
    ...
    public function template() {
        try {
            ...
            // 2. Получение данных
            $res = $this->getEmployeeModel()->getListByRoles(array('10001', 10002));
            $access = $this->getEmployeeModel()->getCommentsAccessList();
            $users = array();
            if(!empty($res)){
                foreach ($res as $row) {
                    $row['Access'] = 0;
    ...
    public function template_update() {
        try {
            ...
            if(!empty($checked)){
                $this->getEmployeeModel()->addCommentsAccessId($id);
            }
            else{
                $this->getEmployeeModel()->removeCommentsAccessId($id);
            }
