...
<div class="panel assol-grey-panel">
    <div class="panel-body">
            ...
            <div class="document-top-btns">
                ...
                <button class="btn assol-btn add right action-article-add" title="Создать статью" style="margin-left: 20px">
                    <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                    Создать статью
                </button>
            </div>
...
<script id="documentListTemplate" type="text/x-jquery-tmpl">
    <div class="document" record="${ID}">
        <div class="document-menu">
            ...
            <?php if ($role['isDirector'] || $role['isSecretary']): ?>
            {{if Content}}
                <span class="glyphicon glyphicon-remove-circle document-menu-btn remove action-article-remove" aria-hidden="true" title="Удалить"></span>
            {{else}}
            ...
            {{if IsFolder==0}}
                {{if Content}}
                <a href="<?=base_url('documents')?>/${Parent}/edit/${ID}">
                    <span class="glyphicon glyphicon-edit document-menu-btn blue" aria-hidden="true" title="Редактировать"></span>
                </a>
                {{else}}
        ...
        <div>
            ...
            <a href="#" class="action-folder-open">
                <img src="<?=base_url('public/img')?>/<?= IS_LOVE_STORY ? 'folder-lovestory' : 'folder' ?>.png">
                <p class="document-name">${Name}</p>
            </a>
            {{else}}
                {{if Content}}
                <a href="<?=base_url('documents')?>/${Parent}/show/${ID}">
                    <img src="<?=base_url('public/img')?>/training.png">
                    <p class="document-name">${Name}</p>
                </a>
                {{else}}
