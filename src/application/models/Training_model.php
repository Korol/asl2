...
class Training_model extends MY_Model {
    ...
    private $table_training_file = "CREATE TABLE `assol_training_file` (
      `ID` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Уникальный номер записи',
      `Name` varchar(256) NOT NULL COMMENT 'Название файла/папки',
      `IsFolder` tinyint(1) DEFAULT '0' COMMENT 'Флаг папки',
      `Parent` int(11) NOT NULL DEFAULT '0' COMMENT 'ID родительского каталога',
    ...
    private $table_training_file_rights = "CREATE TABLE `assol_training_file_rights` (
      `ID` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Уникальный номер записи',
      ...
      CONSTRAINT `assol_training_file_rights_ibfk_1` FOREIGN KEY (`DocumentID`) REFERENCES `assol_document` (`ID`) ON DELETE CASCADE ON UPDATE NO ACTION,
      CONSTRAINT `assol_training_file_rights_ibfk_2` FOREIGN KEY (`EmployeeID`) REFERENCES `assol_employee` (`ID`) ON DELETE CASCADE ON UPDATE NO ACTION
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Права доступа к файлам';";
    ...
    public function initDataBase() {
        ...
        $this->db()->query($this->table_training_file);
        $this->db()->query($this->table_training_file_rights);
    }
    ...
    public function dropTables() {
        ...
        $this->dbforge->drop_table(self::TABLE_TRAINING_FILE_NAME, TRUE);
        $this->dbforge->drop_table(self::TABLE_TRAINING_FILE_RIGHTS_NAME, TRUE);
    }
    ...
    /**
     * Получить список файлов и папок в указанной папке
     *
     * @param int $parent ID родительского каталога
     *
     * @return mixed
     */
    public function fileGetList($parent) {
        return $this->db()
            ->from(self::TABLE_TRAINING_FILE_NAME)
     ...
     *
     * @param int $id ID файла в системе
     */
    public function fileDelete($id) {
        $record = $this->db()->get_where(self::TABLE_TRAINING_FILE_NAME, ['ID' => $id])->row_array();

        if ($record && empty($record['IsFolder'])) {
            $file = './files/training/'.$record['ID'];
            if (file_exists($file)) unlink($file); // Удаление файла

            $this->db()->delete(self::TABLE_TRAINING_FILE_NAME, ['ID' => $id]); // Удаление записи из таблицы
        } /*else {
            // Рекурсивное удаление вложенных каталогов и документов
     ...
     * @return int ID записи
     */
    public function fileInsert($name, $parent, $idEmployee, $isFolder = true, $content = null) {
        $data = array(
            'Name' => $name,
            'Parent' => $parent ? $parent : 0,
            'IsFolder' => $isFolder,
        ...
        if (empty($isFolder)) {
            ...
            $this->db()->insert(self::TABLE_TRAINING_FILE_NAME, $data);
            $id = $this->db()->insert_id();

            // Пытаемся сохранить в файл
            if (file_put_contents("./files/training/$id", $content) === FALSE) {
    ...
    public function fileGet($id) {
        return $this->db()->get_where(self::TABLE_TRAINING_FILE_NAME, array('ID' => $id))->row_array();
    }
